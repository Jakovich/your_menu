...
(function() {
  ...
  var initiValue = function() {
    for (var i = 0; i < subscribeForm.length; i++) {
      var initSubscribeQuantity = subscribeForm[i].querySelectorAll(".subscribe-variant__input");
      var initpriceSpan = subscribeForm[i].querySelector(".total-sum");
      for (var i = 0; i<initSubscribeQuantity.length; i++ ) {
        if (initSubscribeQuantity[i].checked) {
        var initsubscribeValue = parseInt(initSubscribeQuantity[i].value, 10);
